/**
 * 文件：app/src/main/java/kittoku/osc/preference/accessor/string.kt
 * 作用说明：
 * 本文件封装了对 SharedPreferences 中字符串类型配置项的读取和写入操作，提供统一的访问接口。
 * 主要功能包括：根据配置项键获取字符串值、设置字符串值到 SharedPreferences。
 * 在本项目中，配置管理相关模块（如 PreferenceFragment、配置初始化逻辑等）会调用本文件的方法以读写字符串类型的用户设置。
 * 本文件会调用 SharedPreferences（Android 标准库）、OscPrefKey（配置项枚举）、DEFAULT_STRING_MAP（默认值映射）等模块。
